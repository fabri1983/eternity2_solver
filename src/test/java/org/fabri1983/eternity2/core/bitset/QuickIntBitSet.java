 ...
 * @since   1.0
 */
public class QuickIntBitSet {

	/*
     * BitSets are packed into arrays of "words."  Currently a word is
     * a int, which consists of 32 bits, requiring 5 address bits.
	...
	public QuickIntBitSet(int[] bits) {
		words = bits;
	}
	...
	public void clear(int bitIndex) {
		int wordIndex = wordIndex(bitIndex);
		words[wordIndex] &= ~(1 << bitIndex); // here it seems the compiler does: 1L << (bitIndex & (BITS_PER_WORD - 1))
	}
    ...
    public String toStringAll() {
    	...
    	for (int num : words) {
    		builder.append(intToBinary(num)).append("\n");
    	}
